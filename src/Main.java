...
public class Main extends JPanel implements MouseListener, KeyListener, Runnable {
    ...
    public void init(){
        ...
        balls.add(new Ball(4, 1000,350,true));
        //balls.add(new Ball(5, 500,500,true));
        lines.add(new Line(100, 600,1200,600));
        lines.add(new Line(100, 600,100,100));
        lines.add(new Line(1200, 600,1200,100));
    ...
    public void temp(){
        ...
        balls.get(3).setVelocity(new Vectorio(1300, Math.PI, true));//Max speed 3000 pixels/second
        //balls.get(4).setVelocity(new Vectorio(100,3*Math.PI/4, true));
    }
    public void drawBalls(Graphics g){
        for (int i = 0; i < balls.size(); i++){
            g.drawOval((int) balls.get(i).getX() - 10, (int) balls.get(i).getY() - 10, 20,20);
        }
    }
    ...
    public void checkLineCol(){
        for (int i = 0; i < lines.size(); i++){
            for (int j = 0; j < balls.size(); j++){
                lines.get(i).collision(balls.get(j));
            }
        }
    }
    public void physics (){
    ...
    public void run() {
        ...
        while (true) {
            ...
            if (System.currentTimeMillis() - timer > 1000) {
                ...
                counter = 0;
                System.out.println(balls.get(3).getVelocity());
            }
            /*
            if (System.currentTimeMillis() - timer > 1000){
                timer = System.currentTimeMillis();
